...
let team_generator = function(team_one,team_two,team_one_index,team_two_index,mn,csv,cev,fixed_one,fixed_two,captain_one,captain_two,vice_captain_one,vice_captain_two,selected_tsd,series_index,mode)
{
   ...
   custom_strategy(wk_cnt,bat_cnt,al_cnt,bowl_cnt,selected_team_one,selected_team_two,team_one_index,team_two_index,mn,csv,cev,f1_players,f2_players,c1_players,c2_players,vc1_players,vc2_players,selected_tsd,series_index,mode)
}
let custom_strategy = function(wk_cnt,bat_cnt,al_cnt,bowl_cnt,selected_team_one,selected_team_two,team_one_index,team_two_index,mn,csv,cev,f1_players,f2_players,c1_players,c2_players,vc1_players,vc2_players,selected_tsd,series_index,mode)
{
   c_strategy=[]
   f_wk=0
   f_bat=0
   ...
   if(mode==0)
   {
      ...
      team_generator_helper_one(selected_team_one,selected_team_two,team_one_index,team_two_index,nt,mn,csv,cev,c_strategy,f1_players,f2_players,c1_players,c2_players,vc1_players,vc2_players,f_wk,f_bat,f_al,f_bowl,selected_tsd,series_index)
      })
   }
   else if(mode==1)
   {
...
let team_generator_helper_one = function(st_one,st_two,toi,tti,nt,mn,csv,cev,strategy,f1_players,f2_players,c1_players,c2_players,vc1_players,vc2_players,f_wk,f_bat,f_al,f_bowl,selected_tsd,series_index)
{
  ...
  // console.log(strategy)
   team_generator_helper_two(wk,bat,al,bowl,fp_wk,fp_bat,fp_al,fp_bowl,strategy,toi,tti,nt,mn,csv,cev,c1_players,c2_players,vc1_players,vc2_players,f_wk,f_bat,f_al,f_bowl,selected_tsd,series_index,0)
}
...
let team_generator_helper_two = function(wk,bat,al,bowl,fp_wk,fp_bat,fp_al,fp_bowl,strategy,toi,tti,nt,mn,csv,cev,c1_players,c2_players,vc1_players,vc2_players,f_wk,f_bat,f_al,f_bowl,selected_tsd,series_index,is_custom)
{
   ...
   let hashmap = []
   let selected_id=[]
   wk.forEach((p)=>{selected_id.push(p.player_id)})
   bat.forEach((p)=>{selected_id.push(p.player_id)})
   al.forEach((p)=>{selected_id.push(p.player_id)})
   bowl.forEach((p)=>{selected_id.push(p.player_id)})
   fp_wk.forEach((p)=>{selected_id.push(p.player_id)})
   fp_bat.forEach((p)=>{selected_id.push(p.player_id)})
   fp_al.forEach((p)=>{selected_id.push(p.player_id)})
   fp_bowl.forEach((p)=>{selected_id.push(p.player_id)})
   let temp_cnt =0
   let codervp_cnt=0
   let team_list=[]
  ...
  // console.log(map_cnt)
   ...
   if(is_custom)
   {
      ...
      custom_team_list.forEach((c_team)=>
      {
         ...
         v_team =final_team_creation(c_wk,c_bat,c_al,c_bowl,c_team.captain,c_team.vice_captain,c_team.team_number,c_team.team_credits,0,toi,tti)
